...
import dataSource from '../dist/typeorm/data-source';
...
describe('AppController (e2e)', () => {
  ...
  it('Registrar um novo usuário com role', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/register')
      .send({
        ...authRegisterDTO,
        role: Role.Admin,
        email: 'pauloma123@gmail.com',
      });
  ...
  it('Validar de uma função do novo usuário ainda é user', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
      .set('Authorization', `bearer ${accessToken}`)
      .send();
  ...
  it('Tentar visualizar a lista de usuários', async () => {
    const response = await request(app.getHttpServer())
      .get('/users')
      .set('Authorization', `bearer ${accessToken}`)
      .send();
